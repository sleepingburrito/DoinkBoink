...
void UpdateKeyStates(void) {
	...
	if ( (isAltDown && keyboard[SDL_SCANCODE_F4])
	...
	for (uint8_t i = 0; i < PLAYER_COUNT; ++i) {
		...
		if (0 == pauseIoTimer){
			...
			//update if the controller is seen
			if (SDL_JoystickGetAttached(joyPads[i])) {
